---
layout: post
title: A PLSQL Lexer for Ruby/Rouge
...
This blog is hosted by *Github Pages*. The standard way to generate the site is via *Jekyll* which 
can parse *Markdown* files. It supports code syntax highlighting via a *Ruby* gem named *Rouge*.
...
It renders my repository README.md files fairly well. Unfortunately, it uses a different process than 
*Github Pages* via *Rouge*. The *SQL* lexer for *Rouge* is not as robust as I wanted, and
when I started looking closer, I realize the existing one for Github wasn't great either.

I looked into what I could do about it.

# Rouge

I got a little discouraged at first because I had not picked up *Ruby* before and wasn't
really interested in learning a new language. Nevertheless I dug into it a bit, got
*Ruby*, *Jekyll* and *Rouge* installed locally on a Linux image (apparently it is harder
to do on Windows), and started trying to figure out how it fit together.

I can't tell you how I wound up here, but the documentation for *Rouge* is very good. I found
[Lexer Development](https://rouge-ruby.github.io/docs/file.LexerDevelopment.html). I looked
at the existing *SQL* lexer and also the ones for *Perl* and *Ruby*. 

The code is a state machine that walks through the text using regular
...
it was fairly easy to read and understand what it was doing.

# A PL/SQL Lexer

Long story long enough already, I faked it and wound up making what I believe is a decent 
...
work I've done before. The q-quote operator was a nice challenge that consumed several hours.

I submitted a [Pull Request](https://github.com/rouge-ruby/rouge/pull/1811) to the Rouge
team. There are quite a few in their queue, so I have no idea whether it will fly any time soon,
but I followed the guidelines they provided, so we will see.
